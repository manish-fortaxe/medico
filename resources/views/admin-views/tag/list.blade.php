    ...
    <div class="content container-fluid">
        <div class="mb-3">
            <h2 class="h1 mb-0 d-flex gap-2">
                ...
                <span class="badge badge-soft-dark radius-50 fz-14">{{ $tags->count() }}</span>
            </h2>
        </div>
        <div class="row mt-20">
            <div class="col-md-12">
                <div class="card">
                                ...
                                <form action="{{ url()->current() }}" method="GET">
                                    <div class="input-group input-group-custom input-group-merge">
                                        ...
                                        <input id="datatableSearch_" type="search" name="searchValue" class="form-control"
                                            placeholder="{{ translate('search_by_tag_name') }}" aria-label="{{ translate('search_by_tag_name') }}" value="{{ request('searchValue') }}" required>
                                ...
                                @foreach($tags as $key => $tag)
                                    <tr>
                                        ...
                                        <td class="overflow-hidden max-width-100px">
                                            <span data-toggle="tooltip" data-placement="right" title="{{$tag['tag']}}">
                                                 {{ Str::limit($tag['tag'],20) }}
                                            </span>
                                        </td>
                                        <td class="text-center">{{ Str::limit($tag['tag'],20) }}</td>
                                        <td>
                                            <form action="{{route('admin.brand.status-update') }}" method="post" id="brand-status{{$tag['id']}}-form">
                                                @csrf
                                                <input type="hidden" name="id" value="{{$tag['id']}}">
                                                <label class="switcher mx-auto">
                                                    <input type="checkbox" class="switcher_input toggle-switch-message" name="status"
                                                           id="brand-status{{ $tag['id'] }}" value="1" {{ $tag['status'] == 1 ? 'checked' : '' }}
                                                           data-modal-id = "toggle-status-modal"
                                                           data-toggle-id = "brand-status{{ $tag['id'] }}"
                                                           data-on-image = "brand-status-on.png"
                                                           data-off-image = "brand-status-off.png"
                                                           data-on-title = "{{ translate('Want_to_Turn_ON').' '.$tag['tag'].' '. translate('status') }}"
                                                           data-off-title = "{{ translate('Want_to_Turn_OFF').' '.$tag['tag'].' '.translate('status') }}"
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center gap-2">
                                                <a class="btn btn-outline-info btn-sm square-btn" title="{{ translate('edit') }}"
                                                    href="{{ route('admin.tag.update', [$tag['id']]) }}">
                                                    <i class="tio-edit"></i>
                                                </a>
                    ...
                    @if(count($tags)==0)
                        @include('layouts.back-end._empty-state',['text'=>'no_Tag_found'],['image'=>'default'])
                    @endif
                </div>
            </div>
        </div>
    </div>
    <span id="route-admin-brand-delete" data-url="{{ route('admin.tag.delete') }}"></span>
    <span id="route-admin-brand-status-update" data-url="{{ route('admin.tag.status-update') }}"></span>
    <span id="get-brands" data-brands="{{ json_encode($tags) }}"></span>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content shadow-lg">
                ...
                <div class="modal-body px-4 px-sm-5 pt-0 pb-sm-5">
                    ...
                    <form action="{{ route('admin.tag.delete') }}" method="post" class="product-brand-update-form-submit">
                        @csrf
